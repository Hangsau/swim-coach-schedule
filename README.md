...
- **月曆是主視圖**（與線上頁同一套展開邏輯）：
  - 頭列綠色主按鈕 **「＋ 臨時加一堂」** → 只填課程／班名、日期、時間與可選備註；時間可點選常用值或直接輸入 `08:00-09:00`
  - 臨時單堂**不用先建班**：唯一同名班級自動沿用；新名字自動建立班級並只加這一堂，不會產生每週固定排課
  - 寫入課程後，綠色主操作會自動切到底部「一鍵上線」，依序引導「加課 → 上線」
  - **點一堂課** → 選單：取消這天這堂／挪到別天換時段／這班臨時加一堂／修改班級／刪除排課／刪除班級
  - **點日期空白處** → 選單：在這天臨時加一堂／幫既有班加一堂／建立固定排課的新班級（兩步精靈，中途放棄自動撤銷、不留孤兒班）

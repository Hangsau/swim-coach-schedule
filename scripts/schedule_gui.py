...
def schema_error(data):
    """檢查資料檔是否為 v4 形狀；可用回 None，否則回給教練看的導引文字。"""
    if not isinstance(data, dict):
        return "資料檔格式錯誤：頂層不是 mapping，請檢查 data/schedule.yaml"
    sv = data.get("schema_version")
    if sv != 4:
        return (f"資料檔還是舊版（schema_version={sv}，需要 4）："
                "請先執行 python scripts/migrate_v4.py 完成遷移再開啟")
    return None
...
class ConfirmDialog(tk.Toplevel):
    ...
    def __init__(self, parent, title, resp, on_confirm=None):
        ...
        reverted = resp_data.get("makeups_reverted") or []
        if reverted:
            tk.Label(
                self,
                text=(f"↺ 被刪除的課次原本用來銷帳："
                      f"{', '.join(map(str, reverted))} 已自動回到待補"),
                bg=BG, fg=PROG, font=F_SMALL, anchor="w",
                wraplength=600, justify="left").pack(fill="x")
        synced = resp_data.get("makeups_synced") or []
        if synced:
            tk.Label(
                self,
                text=(f"↺ 這堂是補課：{', '.join(map(str, synced))} "
                      "的補課日已跟著更新"),
                bg=BG, fg=PROG, font=F_SMALL, anchor="w",
                wraplength=600, justify="left").pack(fill="x")
...
class SwimTab(tk.Frame):
    ...
    def refresh(self):
        ...
        def work():
            try:
                data = load_data()
                schema_msg = schema_error(data)
                if schema_msg:
                    self.after(0, lambda: self.sum_lbl.config(
                        text=f"✘ {schema_msg}", fg=BAD))
                    return
    ...
    def _render(self, status, classes, slots, lessons, g_dirty, ahead,
                raw_schedules=None, raw_makeups=None):
        # 摘要
    ...
    def _fields_cancel_lesson(self, class_value="", date_value="", makeup=False):
        return [
            {"flag": "--class", "label": "班級", "kind": "combo",
             "values": self._class_values(), "value": class_value,
             "required": True},
            {"flag": "--date", "label": "取消日期", "kind": "date",
             "value": date_value, "required": True, "hint": "YYYY-MM-DD"},
            {"flag": "--reason", "label": "原因", "kind": "entry",
             "hint": "教練生病、學員請假等"},
            {"flag": "--makeup", "label": "登記為待補課（之後補）", "kind": "check",
             "value": makeup},
        ]

    def _fields_move_lesson(self, class_value="", from_value="", to_value=""):
        return [
            {"flag": "--class", "label": "班級", "kind": "combo",
             "values": self._class_values(), "value": class_value,
             "required": True},
            {"flag": "--from-date", "label": "原本上課日", "kind": "date",
             "value": from_value, "required": True},
            {"flag": "--to-date", "label": "改到哪天", "kind": "date",
             "value": to_value, "required": True, "hint": "只改時間就填同一天"},
            {"flag": "--to-slot", "label": "新時段（常用）", "kind": "combo",
             "values": self._slot_values(), "hint": "不變就留空"},
            {"flag": "--to-time", "label": "新時段（自訂）", "kind": "entry",
             "hint": "HH:MM-HH:MM，不變就留空"},
            {"flag": "--note", "label": "備註", "kind": "entry"},
        ]

    def _fields_fulfill_makeup(self, makeup):
        return [
            {"flag": "--makeup-id", "label": "待補課 ID", "kind": "entry",
             "value": makeup.get("id", ""), "required": True},
            {"flag": "--date", "label": "補課日期", "kind": "date",
    ...
    def _lesson_menu(self, ev, lesson):
        ...
        menu.add_command(
            ...
            state="disabled")
        menu.add_separator()
        menu.add_command(label="取消這堂（不補）",
                         command=lambda: self._form_then_run(
                             "取消這堂", "cancel-lesson",
                             self._fields_cancel_lesson(
                                 class_value=cls_val,
                                 date_value=str(lesson["date"]))))
        menu.add_command(label="取消並登記待補（之後補）",
                         command=lambda: self._form_then_run(
                             "取消並登記待補", "cancel-lesson",
                             self._fields_cancel_lesson(
                                 class_value=cls_val,
                                 date_value=str(lesson["date"]), makeup=True)))
            ...
            menu.add_command(label="這班從某天起換時段…",
                             command=lambda: self._form_then_run(
                                 ...
                                 self._fields_split_schedule(
                                     ...
                                     schedule_id_value=lesson["schedule_id"])))
            ...
            menu.add_command(label="刪除這條排課…",
                             command=lambda: self._form_then_run(
                                 "刪除排課", "remove-schedule", [
                                     {"flag": "--schedule-id", "label": "排課 ID",
                                      "kind": "entry",
                                      "value": lesson["schedule_id"],
                                      "required": True},
                                 ]))
    ...
    def _fields_update_schedule(self, sched):
        """v4 schedule 只是分組 metadata；pattern 欄位留空供重排未來課次。"""
        return [
            {"flag": "--schedule-id", "label": "排課 ID", "kind": "entry",
             "value": sched.get("id", ""), "required": True},
            {"flag": "--start", "label": "重排開始日", "kind": "date",
             "hint": "要重排未來日期才填；只改時間請留空"},
            {"flag": "--end", "label": "結束日", "kind": "date",
    ...
    def _pending_makeups(self, class_id):
        return [m for m in self._makeups
                if m.get("class_id") == class_id
                and m.get("status", "pending") == "pending"]

    def _lesson_actions_menu(self, ev, c, lesson_date, detail_win):
        """單堂課的操作選單：取消（不補 / 登記待補）、挪天、只改時間。"""
        cls_val = f"{c['id']}{SEP}{c.get('name') or ''}"
        dstr = str(lesson_date)
        ...
        menu = tk.Menu(self, tearoff=0, bg=PANEL, fg=FG, activebackground=TRACK,
                       activeforeground=FG, font=F_ROW)
        menu.add_command(
            label=f"{c['id']}　{lesson_date.month}/{lesson_date.day} 這堂",
            state="disabled")
        menu.add_separator()
        menu.add_command(
            label="取消這堂（不補）",
            command=_close_then(lambda: self._form_then_run(
                "取消這堂", "cancel-lesson",
                self._fields_cancel_lesson(class_value=cls_val, date_value=dstr))))
        menu.add_command(
            label="取消並登記待補（之後補）",
            command=_close_then(lambda: self._form_then_run(
                "取消並登記待補", "cancel-lesson",
                self._fields_cancel_lesson(
                    class_value=cls_val, date_value=dstr, makeup=True))))
        menu.add_command(
            label="挪到別天",
            command=_close_then(lambda: self._form_then_run(
                "挪課", "move-lesson",
        ...
        menu.add_command(
            ...
            command=_close_then(lambda: self._form_then_run(
                "改這堂時間", "move-lesson",
                self._fields_move_lesson(
                    class_value=cls_val, from_value=dstr, to_value=dstr))))
        try:
            menu.tk_popup(ev.x_root, ev.y_root)
        finally:
            menu.grab_release()
    ...
    def _render_lesson_chips(self, win, c, future):
        """在排課列底下鋪出實際存在的未來課次，點一個開操作選單。"""
        if not future:
            return

        wrap = tk.Frame(win, bg=BG)
        wrap.pack(fill="x", padx=8, pady=(0, 2))
        cur_row = None
        for i, l in enumerate(future):
            if i % CHIPS_PER_ROW == 0:
                cur_row = tk.Frame(wrap, bg=BG)
                cur_row.pack(fill="x", anchor="w")
            ld = l["date"]
            chip = tk.Label(cur_row, text=f"{ld.month}/{ld.day}", bg=PANEL, fg=FG,
                            font=F_SMALL, padx=6, pady=2)
            chip.pack(side="left", padx=2, pady=2)
            chip.bind("<Button-1>",
                      lambda ev, dd=ld:
                      self._lesson_actions_menu(ev, c, dd, win))
    ...
    def _class_detail(self, c):
        ...
        # --- 待補課帳本 ---
        pending = self._pending_makeups(c["id"])
        if pending:
            tk.Label(win, text=f"⚠ 還欠 {len(pending)} 堂補課（決定補課日後按「補課」銷帳）",
                     bg=BG, fg=BAD, font=(FONT, 11, "bold"), anchor="w").pack(
                fill="x", pady=(2, 3))
            for mk in pending:
                mrow = tk.Frame(win, bg=PANEL)
                mrow.pack(fill="x", pady=1)
                od = mk.get("origin_date") or "?"
                reason = mk.get("reason") or ""
                mtxt = f"{mk.get('id', '')}　原 {od} 停課" + \
                    (f"（{reason}）" if reason else "")
                tk.Label(mrow, text=mtxt, bg=PANEL, fg=FG, font=F_ROW,
                         anchor="w", padx=8, pady=3).pack(
                    side="left", fill="x", expand=True)
                make_btn(mrow, "撤銷",
                         (lambda m=mk: (win.destroy(), self._form_then_run(
                             "撤銷待補", "cancel-makeup",
                             [{"flag": "--makeup-id", "label": "待補課 ID",
                               "kind": "entry", "value": m.get("id", ""),
                               "required": True}]))),
                         color=PANEL).pack(side="right", padx=(4, 6))
                make_btn(mrow, "補課…",
                         (lambda m=mk: (win.destroy(), self._form_then_run(
                             "補課銷帳", "fulfill-makeup",
                             self._fields_fulfill_makeup(m))))).pack(
                    side="right", padx=4)
        ...
        if not cls_scheds:
            ...
            for sched in cls_scheds:
                # v4 schedule 不再儲存 pattern；描述來自 label，日期來自 lessons。
                label_txt = sched.get("label") or "排課"
                ...
                sched_lessons.sort(key=lambda l: (l["date"], l.get("slot_time", "")))
                past = [l for l in sched_lessons if l["date"] < today_d]
                future = [l for l in sched_lessons if l["date"] >= today_d]
                counts_txt = f"已上 {len(past)}／未來 {len(future)}"
                if sched_lessons:
                    first_date = sched_lessons[0]["date"]
                    last_date = sched_lessons[-1]["date"]
                    range_txt = (str(first_date) if first_date == last_date else
                ...
                # 未來各堂日期 chip（點單堂可取消／挪天／改時間）
                self._render_lesson_chips(win, c, future)

        standalone = sorted(
            [l for l in self._all_lessons
             if l.get("class_id") == c["id"] and not l.get("schedule_id")],
            key=lambda l: (l["date"], l.get("slot_time", "")))
        standalone_future = [l for l in standalone if l["date"] >= today_d]
        if standalone:
            tk.Label(
                win,
                text=(f"獨立課次（已上 {len(standalone) - len(standalone_future)}／"
                      f"未來 {len(standalone_future)}）"),
                bg=BG, fg=FG, font=(FONT, 10, "bold"), anchor="w").pack(
                    fill="x", pady=(8, 2))
            self._render_lesson_chips(win, c, standalone_future)
